...
def upgrade() -> None:
    # Only create users table and related objects if not exists
    conn = op.get_bind()
    exists = conn.execute(sa.text(
        "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users');"
    )).scalar()
    if not exists:
        # Create enum type if not exists (normalize case-insensitive check)
        op.execute(
            """
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_type WHERE LOWER(typname) = 'userrole'
    ) THEN
...
"""
        ...
        sa.Column('role', sa.Enum('ANONYMOUS', 'AUTHENTICATED', 'MANAGER', 'ADMIN', name='UserRole', create_constraint=False, native_enum=False), nullable=False),
